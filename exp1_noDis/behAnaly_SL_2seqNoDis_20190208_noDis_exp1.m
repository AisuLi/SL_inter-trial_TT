...
if ~exist('directory','var') || isempty(directory) 
	if ~ispc
		directory = '/Volumes/LASBK/weekendJob/AnalyData/exp';
	else
		addpath('H:\Desktop\mytoolbox');
		savepath;
		directory = 'H:\Desktop\expData\SL_horVer_20181008\Exp1_20181028\Data\AnalyData\exp1_noDis';
	end
end
...
randLocGroupNum  = 2; % 1 = merge[nonPair+pairNonReg]
...
for iMat = 1:size(allfiles,1)
	...
	if mod(str2num(subInfo{1}),2)
		cSubData(cSubData(:,5)==8,9) = 1; %  
		cSubData(cSubData(:,5)==6,9) = regularGroup-1; % after merging two regularities, always 1 = regular 
	else
...
regulTrial = find(allSubData(:,9)~=0); % regular trials
for iRegulTrial = regulTrial' 
	...
	if iRegulTrial>2 % from the 3rd trial, it is possible for the predicting trials to repeat or switch color/shape. 
...
[gm,gsd,gname,corrgcout] = grpstats(allSubData(rowFilter,12),{allSubData(rowFilter,2),allSubData(rowFilter,3)},{'mean','std','gname','numel'});
 gname                   = cellfun(@str2double,gname);
[zscores]                = shiftzs_BCL([corrgcout],0); 

for iRow = 1:size(gname,1)
   
   conFilter = allSubData(:,2) == gname(iRow,1) & allSubData(:,3) == gname(iRow,2) ; % per session per participant
...
% 				[genAccgmCout,genAccgsem,genAccgname,genAccgcout] = grpstats(genAccRaw(:,end-1),{genAccRaw(:,3),genAccRaw(:,4),genAccRaw(:,5)},{'mean','sem','gname','numel'});
	...
	print('-depsc','-painters',fullfile(pwd,[num2str(analyByBlock),'_',num2str(randLocGroupNum),'rdmGrp_',figTitle{iMethod},'_',num2str(oddSubPlot+evenSubPlot-2),'.eps']));
...
% allSubData(allSubData(:,15)<4,15)=1;
...
for iFig = 1:2
	if iFig==1  % regular trials included 
 	...
 	elseif iFig==2  % regular trials excluded  
		rowFilter = allSubData(:,9)~= 1 & ~~allSubData(:,11) & allSubData(:,12)>allSubData(:,14) & allSubData(:,12)<=allSubData(:,13);
		titleName = 'ex';		
	end
	...
	title(['Exp1--regular trials ', titleName 'cluded']);
end

% %%%------ regular pairs are always distance = 4, so consider general other distance=4 trials as baseline 
% rowFilter = ~~allSubData(:,11) & allSubData(:,12)>allSubData(:,14) & allSubData(:,12)<=allSubData(:,13) & allSubData(:,15)==4;
% [gm,gsem,gname,gcout] = grpstats(allSubData(rowFilter,12),{allSubData(rowFilter,2),allSubData(rowFilter,9)},{'mean','sem','gname','numel'});
%  gname                = cellfun(@str2double,gname);
%  disRt                = [gname,gm,gsem,gcout];
% [allSubgmCount,allSubgsem,allSubgname,allSubcorrgcout] = grpstats(disRt(:,end),{disRt(:,2)},{'mean','sem','gname','numel'});
% [allSubgm,allSubgsem,allSubgname,allSubcorrgcout]      = grpstats(disRt(:,end-2),{disRt(:,2)},{'mean','sem','gname','numel'});
...
rowFilter =  ~~allSubData(:,11) & allSubData(:,10) & allSubData(:,12)>allSubData(:,14) & allSubData(:,12)<=allSubData(:,13);
% rowFilter =  ~~allSubData(:,11) & allSubData(:,12)>allSubData(:,14) & allSubData(:,12)<=allSubData(:,13);

[gm,gsem,gname,gcout] = grpstats(allSubData(rowFilter,12),{allSubData(rowFilter,1),allSubData(rowFilter,2),allSubData(rowFilter,3),allSubData(rowFilter,9)},{'mean','sem','gname','numel'});
% [gm,gsem,gname,gcout] = grpstats(allSubData(rowFilter,12),{allSubData(rowFilter,2),allSubData(rowFilter,9)},{'mean','sem','gname','numel'});
...
for iGroup = 1:2
	switch iGroup
		case 1 % odd
			...
			validRtRange  = [500 1000; 700 1200; 400 900; 750 1250; 700 1200; 700 1200; 400 900;... 
                             800 1300; 400 900; 800 1300; 800 1300; 400 900; 600 1100;600 1100;];
		case 2 % even
			nSub          = evenSubPlot; % [1:evenSubPlot]+nColm;
			cGroupValidRt = validRt_even;
			iSub          = [1:nSub 100];
			validRtRange  = [600 1100;400 900;700 1200;600 1100;500 1000;700 1200; 500 1000; 500 1000;500 1000; 800 1300;600 1100; 600 1100;600 1100;; 600 1100;];
	end
...
print('-depsc','-painters',fullfile(pwd,[num2str(analyByBlock),'_',num2str(randLocGroupNum),'rdmGrp_RT_',num2str(oddSubPlot+evenSubPlot-2),'.eps']));
...
[meanOfBlock,validRtgSem,validRtgname,validRtgcout] = grpstats(validRtRaw(:,end-2),{validRtRaw(:,1),validRtRaw(:,2),validRtRaw(:,4)},{'mean','sem','gname','numel'});
RtMeanForSpss = reshape(meanOfBlock,3,numel(unique(validRtRaw(:,2))))';



cd ..
% close all;
save(['result_exp_',num2str(8/analyByBlock),'sess_',num2str(randLocGroupNum),'rdmGrp']);