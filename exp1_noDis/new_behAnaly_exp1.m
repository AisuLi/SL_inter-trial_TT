...
for iMat = 1:size(allfiles,1)
	...
	cSubData(cSubData(:,5)==cPair(2,2),9) = regularGroup-1; % after merging two regularities, always 1 = regular 
...
[gm,gsd,gname,corrgcout] = grpstats(allSubData(rowFilter,12),{allSubData(rowFilter,2),allSubData(rowFilter,4)},{'mean','std','gname','numel'}); %  ,allSubData(rowFilter,9)
...
for iRow = 1:size(gname,1)
	...
	allSubData(conFilter,13) = gm(iRow) + zscores(iRow)*gsd(iRow);  % 
...
mean(corrgcout/512);
mean(corrgcout/512)-2.5*std(corrgcout/512);
...
gname                 = cellfun(@str2double,gname);
trialNum              = repmat([48 16]', numel(unique(gname(:,1)))*numel(unique(gname(:,2))) ,1);
ACC                   = [gname,gcout./trialNum];
accForSpss            = reshape(ACC(:,4),numel(unique(gname(:,2))) * numel(unique(gname(:,3))),numel(unique(gname(:,1))))';

[gm,gsem,gname,gcout] = grpstats(ACC(:,4),{ACC(:,1),ACC(:,3)},{'mean','sem','gname','numel'});
meanOfBlock			  = reshape(gm,2,34)';
...
[gm,gsem,gname,gcout] = grpstats(allSubData(rowFilter,12),{allSubData(rowFilter,2),allSubData(rowFilter,3),allSubData(rowFilter,9)},{'mean','sem','gname','numel'}); % ,allSubData(rowFilter,9)
gname                 = cellfun(@str2double,gname);
disRt                 = [gname,gm,gsem,gcout];
RtForSpss             = reshape(disRt(:,4),16,numel(unique(disRt(:,1))))';

[gm,gsem,gname,gcout] = grpstats(disRt(:,4),{disRt(:,1),disRt(:,3)},{'mean','sem','gname','numel'});
meanOfBlock			  = reshape(gm,2,34)';
