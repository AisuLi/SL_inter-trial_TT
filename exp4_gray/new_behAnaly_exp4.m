...
if ~exist('directory','var') || isempty(directory) 
	if ~ispc
		...
		directory = 'H:\Desktop\expData\SL_horVer_20181008\Exp1_20181028\Data\AnalyData\exp4_gray';
	end
end
...
allSubDataInfo  = {'subGrp';'subNo';'sessionNo';'blockNo';'IsDistra';'targLoc';'distracLoc';'targShape';
				   'disColor';'targLine';'lineCongruent';'regular';'regularPairedAcc';'acc';'rt';'slowLimit';'fastLimit';
					'inter-trial tt distance';'inter-trial targetShape';'inter-trial disColor';'inter-trial target hemisphere';'preTrialCorrect';
...
for iMat = 1:size(allfiles,1)
	...
	for iBlock = 1:BlockNum
		...
		%% ----- column 2/3 = target shape, distractor color repeat(1) or switch(0) -------///
	...
	cSubData(:,[13:15]) = [ones(size(cSubData,1),1) acc(:) responseTimes(:)*1000];
...
[gm,gsd,gname,corrgcout] = grpstats(allSubData(rowFilter,15),{allSubData(rowFilter,2),allSubData(rowFilter,9),allSubData(rowFilter,4)},{'mean','std','gname','numel'}); %  
 gname                   = cellfun(@str2double,gname);
[zscores]                = shiftzs_BCL([corrgcout],0); 

for iRow = 1:size(gname,1)

	conFilter = allSubData(:,2) == gname(iRow,1) & allSubData(:,9) == gname(iRow,2) & allSubData(:,4) == gname(iRow,3) ;
	allSubData(conFilter,16) = gm(iRow) + gsd(iRow)* zscores(iRow);  % zscores(iRow)
...
mean(corrgcout/960);
mean(corrgcout/960)-2.5*std(corrgcout/960);
...
gname                 = cellfun(@str2double,gname);
trialNum              = repmat(3*[72 24 84 28 84 28]', numel(unique(gname(:,1))),1);  %%216 72 252 84 252 84
